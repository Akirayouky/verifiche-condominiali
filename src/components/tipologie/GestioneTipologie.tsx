...
export default function GestioneTipologie() {
  ...
  const handleToggleActive = async (id: string, attiva: boolean): Promise<boolean> => {
    ...
    const updateData: UpdateTipologiaRequest = {
      ...
      campiPersonalizzati: tipologia.campiPersonalizzati,
      attiva
    }
  ...
  const stats = {
    ...
    conCampi: tipologie.filter(t => t.campiPersonalizzati.length > 0).length
  }
      ...
      {editingTipologia && (
        <TipologiaForm
          ...
          initialData={{
            ...
            campiPersonalizzati: editingTipologia.campiPersonalizzati,
            attiva: editingTipologia.attiva
          }}
          isEdit={true}
        />
