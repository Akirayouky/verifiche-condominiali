...
export const getConnection = async (): Promise<mysql.Connection> => {
  try {
    ...
    throw new Error('Impossibile connettersi al database MySQL. Verifica che il server sia avviato e le credenziali siano corrette.')
  }
}
...
export const executeQuery = async <T = any>(
  ...
  try {
    ...
    // Se il database non è disponibile, fornisci un messaggio più chiaro
    if ((error as any).code === 'ECONNREFUSED') {
      throw new Error('Database MySQL non raggiungibile. Controlla che MySQL sia avviato e configurato correttamente.')
    }
...
export const testConnection = async (): Promise<boolean> => {
  try {
    ...
    console.error('❌ Errore connessione database:', error)
    if ((error as any).code === 'ECONNREFUSED') {
      console.error('💡 Suggerimento: Verifica che MySQL sia avviato e che le credenziali in .env.local siano corrette')
    }
    return false
  }
}
...
// Fallback per quando MySQL non è disponibile
